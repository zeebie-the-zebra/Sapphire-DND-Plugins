...
Clue and evidence tracker for investigative campaigns. Track clues
discovered, red herrings, evidence connections, and mystery resolution
state. Surfaces unresolved clues and flags forgotten leads.
...
logger = logging.getLogger("dnd-mystery")
...
def _state():
    return plugin_loader.get_plugin_state("dnd-mystery")
...
def mystery_create(
    ...
    """
    Create a new mystery or investigative thread.

    Args:
        name: Internal name/ID for this mystery (use underscores, e.g. "murder_at_the_inn")
        description: What the party is trying to uncover
        is_active: Whether this is the current active mystery. Default: True
    ...
    data = {
        ...
        "is_active": is_active,
        "status": "open",  # open | in_progress | solved | abandoned
        "clues": {},        # clue_id -> clue data
        "red_herrings": {}, # herring_id -> herring data
        "connections": {},   # {clue_id: [connected_clue_ids]}
        "suspects": {},     # suspect_id -> suspect data
        "notes": "",
        "created_session": "",
    }
...
def clue_add(
    ...
    Args:
        mystery_id: The mystery this clue belongs to
        clue_name: Short identifier (e.g. "bloody_letter", "witness_statement_1")
        clue_text: Full description of the clue
        clue_type: Type — physical | testimony | document | circumstantial | environmental. Default: physical
        discovered_by: Who found it. Default: ''
        location: Where it was found. Default: ''
        significance: How important — minor | key | critical. Default: minor

    Returns:
        Confirmation with clue ID.
    """
...
def clue_reveal(
    ...
    """
    Mark a clue as revealed to the party (they've learned about it in play).

    Args:
        mystery_id: The mystery
        clue_name: The clue ID to reveal
...
def clue_connect(
    ...
    """
    Create a logical connection between two clues (A relates to B).
    Helps track how clues link together in the party's understanding.

    Args:
        mystery_id: The mystery
        clue_a: First clue ID
        clue_b: Second clue ID
        connection_note: What the connection is. Default: ''
...
def red_herring_add(
    ...
    """
    Add a red herring to a mystery — a clue that seems important but isn't.

    Args:
        mystery_id: The mystery
        herring_name: Short identifier
        herring_text: What the false lead appears to show
        planted_by: Who planted it (NPC name). Default: ''
        revealed_as_herring: Whether the party has learned it's a false lead. Default: False
...
def red_herring_reveal(
    mystery_id: str,
    herring_name: str
) -> str:
    """
    Mark a red herring as revealed to the party.

    Args:
        mystery_id: The mystery
        herring_name: The herring ID
...
def suspect_add(
    ...
    """
    Add a suspect to a mystery.

    Args:
        mystery_id: The mystery
        suspect_name: NPC name
        description: Brief description. Default: ''
        alibi: Their stated alibi. Default: ''
        motive: Possible motive. Default: ''
        evidence_against: What evidence points to them. Default: ''
...
def suspect_rulout(mystery_id: str, suspect_name: str, reason: str = "") -> str:
    """
    Rule a suspect out (cleared or eliminated).

    Args:
        mystery_id: The mystery
        suspect_name: Suspect ID
        reason: Why they were cleared. Default: ''
...
def mystery_status(mystery_id: str = "") -> str:
    """
    Get full status of a mystery — clues, suspects, red herrings, connections.

    Args:
        mystery_id: The mystery to check. If empty, shows all mysteries.

    Returns:
        Formatted mystery status.
    """
    if mystery_id:
        return _mystery_detail(mystery_id)

    # Show index of all mysteries
    index = _load_index()
    if not index:
...
def mystery_solve(mystery_id: str, resolution: str = "") -> str:
    """
    Mark a mystery as solved, with a resolution summary.

    Args:
        mystery_id: The mystery to close
        resolution: What the party discovered / the truth. Default: ''
...
def mystery_delete(mystery_id: str) -> str:
    """
    Delete a mystery and all its data.

    Args:
        mystery_id: The mystery to delete
...
def mystery_prompt() -> str:
    """
    Return active mystery state for prompt injection.

    Returns:
        One-line summary of active mysteries, or empty string.
    """
    index = _load_index()
    if not index:
        return ""
    ...
    for mid in index:
        ...
        if status == "solved":
            continue
        clue_count = len([c for c in data.get("clues", {}).values() if c.get("revealed_to_party")])
        active_parts.append(f"{mid}({status}, {clue_count} clues)")

    if not active_parts:
        return ""
    return f"🔍 Active mysteries: {', '.join(active_parts)}"
