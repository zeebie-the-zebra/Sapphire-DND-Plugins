...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "resource_use",
            "description": (
                "Mark one or more uses of a resource as spent. "
                "Call this the moment a resource is used — rage activated, ki spent, "
                "bardic inspiration given, channel divinity used, etc. "
                "Do not wait until after the effect."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def execute(function_name, arguments, config):
    ...
    if function_name == "resource_setup":
        ...
        if not targets:
            return "No resources set up for any characters.", True
        ...
        _save(data)
        return "\n".join(lines) if len(lines) > 1 else f"No resources needed restoring.", True
        ...
        for name, char_data in data.items():
            lines.append(f"\n{name} (L{char_data.get('level','?')} {char_data.get('class','?')}):")
