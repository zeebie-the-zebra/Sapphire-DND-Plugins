...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Begin combat. Roll initiative for all combatants and set up the initiative order. Call this at the start of every fight.",
            "parameters": {
                "type": "object",
                "properties": {
                    "combatants": {
...
def execute(function_name, arguments, config):
    ...
    # ── encounter_start_combat ──
        ...
        if not combatants_raw:
            return "No combatants provided.", False
        ...
        for i, c in enumerate(combatants):
            ...
            tag = " (PC)" if c["is_player"] else ""
            lines.append(f"{i+1}. **{c['name']}{tag}** — Initiative {c['initiative']} | HP {c['hp']}/{c['hp_max']}{marker}")
        return "\n".join(lines), True
    ...
    # ── encounter_next_turn ──
        ...
        for upd in hp_updates:
            for c in combatants:
                if c["name"].lower() == upd["name"].lower():
                    c["hp"] = upd["hp"]

        # Remove dead enemies (keep dead PCs for death saves)
        alive = [c for c in combatants if c["hp"] > 0 or c["is_player"]]
        ...
        for i, c in enumerate(alive):
            ...
            status = f"HP {c['hp']}/{c['hp_max']}"
            if c["hp"] == 0: status = "💀 DOWN"
            conds = f" [{','.join(c['conditions'])}]" if c.get("conditions") else ""
            lines.append(f"  {'→' if i == next_turn else ' '} {c['name']}: {status}{conds}{marker}")
        return "\n".join(lines), True
    ...
    # ── encounter_combat_status ──
        ...
        for i, c in enumerate(combatants):
            ...
            if c["hp"] == 0: status = "💀 DOWN"
            conds = f" [{','.join(c['conditions'])}]" if c.get("conditions") else ""
            lines.append(f"  {i+1}. **{c['name']}**: {status}{conds}{marker}")
        return "\n".join(lines), True
