...
Reads from (best-effort, gracefully skips missing plugins):
  - dnd-scaffold        (characters, combat, resources, conditions)
  - dnd-campaign        (campaign, quests)
  - dnd-time            (game clock)
  - dnd-scene           (current scene/location)
  - dnd-weather         (weather conditions)
  - dnd-threads          (narrative threads)
  - dnd-npc-relations    (NPC attitudes)

Supports multi-campaign: reads campaign-scoped data from dnd-scaffold.
"""
...
def status_get_all(include_npcs: bool = False) -> str:
    ...
    # ── WEATHER ───────────────────────────────────────────────────────────────
    try:
        wx = _get("dnd-weather", f"weather_state:{campaign_id}") or _get("dnd-weather", "weather_state") or {}
        # weather.py stores as {"default": {...}, "locations": {...}}
        if "default" in wx:
            wx = wx["default"]
        if wx:
    ...
    # ── PARTY ─────────────────────────────────────────────────────────────────
    try:
        # Read from dnd-scaffold (the canonical character store in the scaffold)
        ...
        for char in chars.values() if isinstance(chars, dict) else []:
            ...
            inv_str = f" | Inv: {', '.join(i['item'] for i in inv)}" if inv else ""
            char_lines.append(
                f"{char.get('name', '?')}: HP {hp}/{hp_max} | AC {ac}{cond_str}{ds_str}{inv_str}"
            )
