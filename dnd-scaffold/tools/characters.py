...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "REQUIRED — Apply damage to a character. Call this EVERY TIME the narrative describes a character taking damage, losing HP, or being hit. If you wrote 'Thorin takes 8 damage', 'the spell deals 15 hit points to Zara', 'she is hit for 6', 'the trap deals 3d10 damage', or 'he falls to 0 HP' — you MUST call character_damage NOW. This handles temp HP absorption first, then real HP. When HP reaches 0, death saves begin automatically. Do NOT describe damage in narration without calling this tool.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name":   {"type": "string",  "description": "Character name"},
    ...
    },
    {
        ...
        "function": {
            "name": "character_heal",
            "description": "REQUIRED — Heal a character or add temporary HP. Call this EVERY TIME the narrative describes a character being healed, recovering HP, or gaining temp HP. If you wrote 'Zara heals for 10 HP', 'he drinks a potion and recovers 5 hit points', 'the cure wound spell restores 15 HP', 'she gains 8 temporary HP', or 'the healing word restores 6 HP' — you MUST call character_heal NOW. Also use this after rest_long (is_rest='long') or rest_short (is_rest='short') calls to apply HP recovery.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name":    {"type": "string",  "description": "Character name"},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "REQUIRED — Expend a spell slot when a character casts any levelled spell. Call this EVERY TIME the narrative describes a character casting a spell of level 1-9. If you wrote 'Zara casts Magic Missile', 'he casts Fireball at 3rd level', 'she uses a 2nd-level slot for Guiding Bolt', or 'the spell is cast' — you MUST call character_use_spell_slot NOW with the correct slot level. If you narrated a spell being cast without calling this tool, call it immediately after. This applies to all casters: Wizard, Cleric, Sorcerer, Bard, Druid, Paladin, Ranger, Warlock.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name":  {"type": "string",  "description": "Character name"},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "REQUIRED — Add an item to a character's inventory. Call this EVERY TIME the narrative describes a character obtaining any item. This includes: loot from a chest/body/room, shop purchases, NPC gifts, treasure finds, rewards, found coins, picked pockets, or anything else acquired. If you wrote 'Grog takes the sword', 'the merchant gives her a potion', 'they found 50 gold', 'she picks up the amulet', or 'he receives a reward' — you MUST call character_add_item NOW before continuing. Do NOT wait until end of session or end of combat. Include notes like '+1', 'cursed', 'stolen', etc.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name":     {"type": "string",  "description": "Character name"},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "REQUIRED — Remove an item from a character's inventory. Call this EVERY TIME the narrative describes an item being lost, stolen, sold, consumed, given away, dropped, or destroyed. If you wrote 'Grog sells the sword', 'the potion is drunk', 'her ring is stolen', 'he drops the shield', 'they lost the key', or 'the item breaks' — you MUST call character_remove_item NOW. This includes gold being spent. Do NOT wait until end of session.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name":     {"type": "string",  "description": "Character name"},
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "REQUIRED — Add or remove a condition from a character. Call this EVERY TIME the narrative describes a character becoming or ceasing to be affected by any D&D condition. If you wrote 'Zara is now poisoned', 'Thorin falls unconscious', 'she becomes paralyzed', 'he is no longer blinded', 'the creature is restrained', or 'they become frightened' — you MUST call character_set_condition NOW with active=True to apply, or active=False to remove. Conditions include: blinded, charmed, deafened, exhaustion, frightened, grappled, incapacitated, invisible, paralyzed, petrified, poisoned, prone, restrained, stunned, unconscious, and concentration.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name":      {"type": "string",  "description": "Character name"},
...
def _get_campaign_id(config=None) -> str:
    ...
    try:
        ...
        if guild_id:
            # Check guild-scoped key first, fall back to non-guild
            val = campaign_state.get(f"active_campaign:{guild_id}")
            if not val:
                val = campaign_state.get("active_campaign")
            if val:
                return val
        else:
            val = campaign_state.get("active_campaign")
...
def _migrate_if_needed(campaign_id: str):
    ...
    # Migrate legacy character data (legacy = non-scoped "characters" key)
    legacy = state.get("characters")
    if legacy:
        # Only migrate if campaign doesn't already have its own characters
        existing = state.get(f"characters:{campaign_id}")
        if not existing:
