"""
dnd-scaffold — tools/homebrew.py

Wrapped from dnd-homebrew. Pre-execute signature changed from:
...
def _auto_create_character_entry(args: dict, campaign_id: str):
    ...
    logger.info(f"[dnd-scaffold] Auto-created dnd-characters entry: {linked_name}")
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "content": {"type": "string", "description": "The full stat block, item description, spell details, or character backstory."},
                    "tags": {"type": "array", "items": {"type": "string"}, "description": "Optional tags for filtering and search"},
                    "cr": {"type": "string", "description": "Challenge Rating (monsters only)"},
                    "rarity": {"type": "string", "description": "Rarity (items only)"},
                    "spell_level": {"type": "integer", "description": "Spell level (spells only, 0-9)"},
                    "character_class": {"type": "string", "description": "Class (characters only)"},
                    "character_level": {"type": "integer", "description": "Level (characters only, 1-20)"},
                    "character_race": {"type": "string", "description": "Race (characters only)"},
                    ...
                    "character_stats": {"type": "object", "description": "Ability scores (characters only)"},
                    "character_alignment": {"type": "string", "description": "Alignment (characters only)"},
                    "character_background": {"type": "string", "description": "Background (characters only)"},
                    "linked_character": {"type": "string", "description": "Name of corresponding dnd-characters entry (characters only)"},
                },
...
def _homebrew_add(args: dict, campaign_id: str) -> tuple:
    ...
    logger.info(f"[dnd-scaffold] Added {type_}: {name}{extra_str} in campaign {campaign_id}")
...
def _homebrew_update(args: dict, campaign_id: str) -> tuple:
    ...
    _save_all(campaign_id, data)
    logger.info(f"[dnd-scaffold] Updated {type_}: {name} in campaign {campaign_id}")
...
def _homebrew_delete(args: dict, campaign_id: str) -> tuple:
    ...
    logger.info(f"[dnd-scaffold] Deleted {type_}: {name} from campaign {campaign_id}")
